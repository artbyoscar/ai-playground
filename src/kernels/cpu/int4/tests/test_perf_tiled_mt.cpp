...
int main() {
  ...
  std::vector<int> NCs = {4,8,16};

  for (int T : Ts) {
    for (int NC : NCs) {
      // warmup
      for (int w=0; w<3; ++w)
        qgemm_int4_fp16_tiled_mt(A_fp16.data(), K, B_packed.data(), B_scales.data(), 0,
                                 C_fp16.data(), N, M, N, K, G, T, NC);
      ...
      std::cout << "T="<<T<<"  NC="<<NC<<"  avg="<<ms<<" ms  ~"<<gflops_per_s<<" GFLOP/s\n";
    }
  }
  return 0;
}
